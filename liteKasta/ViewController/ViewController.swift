    ...
    func listAdapter(_ listAdapter: ListAdapter, sectionControllerFor object: Any) ->
        ListSectionController {
            ...
            if object is Campaign{
                controller = ListSingleSectionController(cellClass: CampaignCell.self, configureBlock: { (item, cell) in
                ...
                controller = ListSingleSectionController(cellClass: FutureCamapignCell.self, configureBlock: { (item, cell) in
                    
                    let campaignCell = cell as! FutureCamapignCell
                    let campaign = item as! FutureCampaign
                    
                    campaignCell.futureCamapignBanner(futureCamapignData: campaign)
                }, sizeBlock: { (item, context) -> CGSize in
                    
                    let width = context!.insetContainerSize.width - 32
                    let height = CGFloat(289)
        ...
        provider.request(.campaigns) { result in
            do {
                ...
                case .success(let response):
                    ...
                    let (activeCampaigns, futureCampaigns, _) = campaigns.filterActive(for: Date())
                    var allCampaigns = [ListDiffable]()
                    let viewModels = activeCampaigns.map() { return Campaign(with: $0) }.filter({$0.codename != "black"})
                    allCampaigns.append(contentsOf: viewModels)
                    let futureCampaign = futureCampaigns.map() { return Campaign(with: $0) }.filter({$0.codename != "black"})
                    let futureViewModel = FutureCampaign(futureCampaign: futureCampaign)
                    allCampaigns.insert(futureViewModel, at: 3)
                    self.state = .success(items: allCampaigns)
